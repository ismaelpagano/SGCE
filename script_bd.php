<?php
    ...
    // $q = $sql->query("SELECT DISTINCT id_compra FROM items_adjudicacion WHERE tipo_doc_prov = 'R' and nro_doc_prov = '215036060012'");

    // $compras = Array();

    // if($q){
    //     while($r = $q->fetch_object()){
    //         $compras[$r->id_compra] = $r->id_compra;
    //     }
    // }
    ...
    //     $query = "UPDATE gestion_compras SET estado_interno = 5 WHERE id_compra = '$compra'";
    //     $q = $sql->query($query);
    ...
    define('DATABASE_COMPRAS', 'gestor_compras_estatales');
    ...
    function actualizador_compras($fecha_inicial, $fecha_fin){
        ...
        print_r("Se actualizará la lista de compras. ".date('Y-m-d H:i:s', time()).".<br>");
        ...
        foreach($compras as $compra){
            ...
            if((!array_key_exists((string)$id_compra, $id_compras_bd)) && (!array_key_exists((string)$id_compra, $id_compras_adj)) ){
                ...
                insert_gestion_compra_bd($id_compra, $attributes['estado_compra'], $attributes['fecha_publicacion'] , $attributes['fecha_ult_mod_llamado']);
                ...
                update_compra_bd($id_compra, $attributes['estado_compra'], $attributes['fecha_ult_mod_llamado']);
    ...
    function insert_gestion_compra_bd($id_compra , $estado_compra , $fecha_publicacion, $fecha_ult_mod_arce){

        $fecha_publicacion = cadena_fecha_hora($fecha_publicacion);
        $fecha_ult_mod_arce = cadena_fecha_hora($fecha_ult_mod_arce);
        $fecha_ult_mod_sgce = date('Y-m-d H:i', time());

        $query = "INSERT INTO gestion_compras ( id_compra , estado_compra , estado_interno , valoracion_interna , fecha_publicacion_llamado , fecha_ult_mod_arce , fecha_ult_mod_sgce ) VALUES ( '".$id_compra."' , '".$estado_compra."' , 0 , 0 , '".$fecha_publicacion."' , '".$fecha_ult_mod_arce."' , '".$fecha_ult_mod_sgce."' )";
    ...
    actualizador_compras('2022-11-17 00:00:00', '2022-11-17 23:59:59');
